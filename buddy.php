...
END;
    $userid = $user['id'];
    $unread_temp = mysql_query("SELECT COUNT(`read`) FROM `{$_PREFIX}pms` WHERE `to`=$userid AND `read`=0 GROUP BY `read` ");
    ...
    print "<br />";
    $pmresult = mysql_query("SELECT `id`,`from`,`title`,`read` FROM `{$_PREFIX}pms` WHERE `to`=" . $user['id'] . " ORDER BY id DESC LIMIT 10", $db);
...
END;
    $post_results = mysql_query("SELECT `id`,`board`,`title` FROM `{$_PREFIX}topics` ORDER BY `lastpost` DESC LIMIT 5");
...
END;
    ...
    $day_results = mysql_query("SELECT `id`,`title` FROM `{$_PREFIX}calendar` WHERE `day`='" . $today . "'");
    $events = "";
    ...
    $day_results = mysql_query("SELECT `id`,`title` FROM `{$_PREFIX}calendar` WHERE `day`='" . $tomorrow . "'");
    $events = "";
    while ($query_row = mysql_fetch_array($day_results)) {
        $events = $events . "- " . $query_row['title'] . "<br />\n";
    }
    print "<a href=\"calendar.php?view=date&amp;day=$tomorrow\">{$_PWNDATA['cal']['tomorrow']}</a>:<br />$events";
    $day_results = mysql_query("SELECT `id`,`title` FROM `{$_PREFIX}calendar` WHERE `day`='" . $dayafter . "'");
    $events = "";
    while ($query_row = mysql_fetch_array($day_results)) {        $events = $events . "- " . $query_row['title'] . "<br />\n";
    }
